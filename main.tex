...
我们假设有嵌入网络$g_\theta : d_x \mapsto d_z$，辅助控制网络$g_\phi$，Koopman算子矩阵$A, B$和恢复矩阵$C$。
下面介绍网络架构和前馈过程：
...
\end{equation}
% 在上面的等式中，将原始系统状态和提升后的状态和原始状态拼接在一起，
% 这样做的原因除了在升维空间中能够强调系统原始状态，
% 还能够通过恢复矩阵还原系统原始状态：
% \begin{equation}
%   x_t = C z_t
% \end{equation}
...
在此基础上，我们考虑如等式中的 (\ref{Koopman-critic}) Koopman-Critic函数，
...
在上面的修改下，我们依然可以遵从算法 $\ref{sac}$ 对参数网络进行迭代。
