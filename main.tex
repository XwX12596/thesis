...
尽管取得了上述进展，深度强化学习解决方案通常需要巨大计算资源来训练和泛化，
...
而马尔可夫性质则表示，已知当前时刻状态$S_t$时，下一时刻状态$S_{t + 1}$仅与$S_t$有关，用$P( S_{t + 1} | S_t)$表示。而从前一个状态经过随机进入下一个状态的过程被称为状态转移。
...
\end{equation}
等式左边为价值函数处于时间不$t$时对于价值的估计，等式右边是对于左边的一个无偏估计，
...
此外，本章中还提出一种Koopman算子和强化学习交替训练的训练架构，更好地发挥神经网络的优势
