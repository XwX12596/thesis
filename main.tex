...
\title{{基于深度Koopman算子的}{非线性系统强化学习研究}}

\entitle{{ Deep Koopman Operator Based }{ Reinforcement Learning of Nonlinear System }}
...
%中文摘要
\ZhAbstract{这是测试的参考文献\cite{partl2016}。}
...
\section{国内外研究现状}
\subsection{强化学习}
强化学习（RL）是一个快速发展的领域，处于机器学习和控制理论的交叉点，其中智能代理学习如何与复杂的环境进行交互以实现目标[1,2]。
深度强化学习（DRL）[3-9]最近被证明能够在几个具有挑战性的任务中实现人类水平或超人类的表现，包括玩视频游戏[10,11]和策略游戏[12-14]。
DRL也越来越多地用于科学和工程应用，包括用于药物发现[15]、机器人操作[16]、自动驾驶[17]和无人机赛车[18]、流体流动控制[19-25]和聚变控制[26]。
尽管取得了这一进展，DRL解决方案通常需要巨大的计算资源来训练和泛化，并且它们通常缺乏在许多应用中至关重要的可解释性。
许多强化学习策略背后的算法与优化和最优控制理论[2]中的贝尔曼和Hamilton-Jacobi-Bellman方程密切相关。
然而，求解这些方程对于高维非线性系统来说变得棘手，这通常会激发深度学习的使用。
在本文中，我们通过库普曼算子[27-33]的新应用重新审视贝尔曼方程，这允许我们在infinite-dimensional函数空间上将非线性动态系统重塑为线性方程组。
...
% 在这项工作中，我们提出了一个端到端深度学习框架，
% 将Koopman嵌入函数和Koopman运算符一起学习，以缓解这样的困难。
% 我们首先用神经网络参数化嵌入函数和Koopman运算符，并用K步损失函数端到端训练它们。
% 然后，增强了一个辅助控制网络，对非线性状态相关控制项进行编码，以对控制输入中的非线性进行建模。
% 该编码项被认为是新的控制变量，以确保嵌入系统中建模系统的线性。
% 我们接下来将线性的二次调节器（LQR）部署在线性的嵌入空间上，
% 以导出最优控制策略并解码来自控制网络的实际控制输入。
...
\begin{table}[H]
  ...
  \begin{tabular}{ccccc}
    ...
    \midrule
    2000步回报 & -724204 & -156293 & -74895 & -33071 \\
    \bottomrule
  \end{tabular}
